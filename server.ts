...
async function startServer() {
  ...
  app.post('/api/prorated-rules', (req, res) => {
    try {
      ...
      const newRule: ProratedBudgetRule = {
        id,
        name: body.name,
  ...
  app.post('/api/savings-goals/:id/contributions', (req, res) => {
    try {
      ...
      const historyItem = {
        ...
        note: note || undefined,
        type: (type || 'deposit') as 'deposit' | 'withdrawal',
      };
  ...
  // --- Recurring Items CRUD & Execution ---
  ...
  app.post('/api/recurring/apply', (req, res) => {
    try {
      ...
      for (const rec of allRecurring) {
        ...
        const dayStr = String(Math.min(28, rec.dayOfMonth)).padStart(2, '0');
        ...
        const newTx: Transaction = {
          ...
          notes: `Auto-generated recurring item for ${targetMonth}`,
        };
      ...
      res.json({ success: true, addedCount, month: targetMonth });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });
