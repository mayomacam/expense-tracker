var __defProp=Object.defineProperty;var __name=(target,value)=>__defProp(target,"name",{value,configurable:true});import initSqlJs from"sql.js";import fs from"fs";import path from"path";import{getInitialSeedData,getDemoSeedData}from"../data/seedData";let db=null;const DB_DIR=path.join(process.cwd(),"data");const DB_FILE=path.join(DB_DIR,"budget.sqlite");function persistDb(){if(!db)return;try{if(!fs.existsSync(DB_DIR)){fs.mkdirSync(DB_DIR,{recursive:true})}const data=db.export();const buffer=Buffer.from(data);fs.writeFileSync(DB_FILE,buffer)}catch(err){console.error("Failed to persist SQLite database to disk:",err)}}__name(persistDb,"persistDb");function query(sql,params=[]){if(!db)return[];const stmt=db.prepare(sql);stmt.bind(params);const results=[];while(stmt.step()){results.push(stmt.getAsObject())}stmt.free();return results}__name(query,"query");function run(sql,params=[]){if(!db)return;db.run(sql,params);persistDb()}__name(run,"run");async function initDatabase(){if(db)return db;const SQL=await initSqlJs();if(!fs.existsSync(DB_DIR)){fs.mkdirSync(DB_DIR,{recursive:true})}if(fs.existsSync(DB_FILE)){try{const filebuffer=fs.readFileSync(DB_FILE);db=new SQL.Database(filebuffer);console.log("\u2705 SQLite database loaded successfully from disk:",DB_FILE)}catch(e){console.error("Error loading existing SQLite db file, creating new:",e);db=new SQL.Database}}else{db=new SQL.Database;console.log("\u{1F331} Creating new SQLite database at:",DB_FILE)}createSchema();seedIfEmpty();return db}__name(initDatabase,"initDatabase");function createSchema(){if(!db)return;db.run(`
  ...
  `)}__name(createSchema,"createSchema");function insertDemoData(demo){for(const cat of demo.categories){run(`INSERT OR REPLACE INTO categories (id, name, icon, color, monthlyBudget, isCustom) VALUES (?, ?, ?, ?, ?, ?)`,[cat.id,cat.name,cat.icon,cat.color,cat.monthlyBudget||0,cat.isCustom?1:0])}for(const tx of demo.transactions){run(`INSERT OR REPLACE INTO transactions (id, title, amount, type, category, date, tags, notes, paymentMethod, isRecurring, recurringFrequency, receiptUrl)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[tx.id,tx.title,tx.amount,tx.type,tx.category,tx.date,JSON.stringify(tx.tags||[]),tx.notes||null,tx.paymentMethod,tx.isRecurring?1:0,tx.recurringFrequency||null,tx.receiptUrl||null])}for(const rule of demo.proratedRules){run(`INSERT OR REPLACE INTO prorated_rules (id, name, categoryId, targetTags, monthlyMaxSpend, month, rolloverEnabled, rolloverAmount, alertThresholdPercent, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[rule.id,rule.name,rule.categoryId||null,JSON.stringify(rule.targetTags||[]),rule.monthlyMaxSpend,rule.month,rule.rolloverEnabled?1:0,rule.rolloverAmount||0,rule.alertThresholdPercent||100,rule.notes||null])}for(const goal of demo.savingsGoals){run(`INSERT OR REPLACE INTO savings_goals (id, name, targetAmount, currentAmount, targetDate, icon, color, category, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,[goal.id,goal.name,goal.targetAmount,goal.currentAmount,goal.targetDate,goal.icon||null,goal.color||null,goal.category||null,goal.notes||null]);if(goal.history&&goal.history.length>0){for(const h of goal.history){run(`INSERT OR REPLACE INTO savings_history (id, goalId, date, amount, note, type) VALUES (?, ?, ?, ?, ?, ?)`,[h.id,goal.id,h.date,h.amount,h.note||null,h.type])}}}for(const debt of demo.debts){run(`INSERT OR REPLACE INTO debts (id, name, lenderName, debtType, totalPrincipal, remainingBalance, interestRate, minimumPayment, dueDay, notes, color)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[debt.id,debt.name,debt.lenderName||null,debt.debtType||"borrowed",debt.totalPrincipal,debt.remainingBalance,debt.interestRate,debt.minimumPayment,debt.dueDay,debt.notes||null,debt.color||null]);if(debt.payments&&debt.payments.length>0){for(const p of debt.payments){run(`INSERT OR REPLACE INTO debt_payments (id, debtId, date, amount, principalPaid, interestPaid, note) VALUES (?, ?, ?, ?, ?, ?, ?)`,[p.id,debt.id,p.date,p.amount,p.principalPaid,p.interestPaid,p.note||null])}}}for(const rec of demo.recurring){run(`INSERT OR REPLACE INTO recurring_items (id, title, amount, type, category, frequency, dayOfMonth, autoApply, tags, paymentMethod, lastAppliedMonth, isActive)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[rec.id,rec.title,rec.amount,rec.type,rec.category,rec.frequency,rec.dayOfMonth,rec.autoApply?1:0,JSON.stringify(rec.tags||[]),rec.paymentMethod,rec.lastAppliedMonth||null,rec.isActive?1:0])}run(`INSERT OR REPLACE INTO user_settings (id, currency, currencyCode, pushNotificationsEnabled, dailyBudgetAlertThreshold, monthlyBudgetWarningThreshold, enableRolloverByDefault, selectedMonth, userName)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,["default",demo.settings.currency,demo.settings.currencyCode,demo.settings.pushNotificationsEnabled?1:0,demo.settings.dailyBudgetAlertThreshold,demo.settings.monthlyBudgetWarningThreshold,demo.settings.enableRolloverByDefault?1:0,demo.settings.selectedMonth,demo.settings.userName])}__name(insertDemoData,"insertDemoData");function populateDemoData(){if(!db)return;console.log("\u{1F504} Wiping and populating demo dataset in SQLite...");run("DELETE FROM transactions");run("DELETE FROM deleted_transactions");run("DELETE FROM categories");run("DELETE FROM prorated_rules");run("DELETE FROM savings_goals");run("DELETE FROM savings_history");run("DELETE FROM debts");run("DELETE FROM debt_payments");run("DELETE FROM recurring_items");run("DELETE FROM read_alerts");run("DELETE FROM user_settings");const demo=getDemoSeedData();insertDemoData(demo);persistDb();console.log("\u2705 Demo dataset populated successfully.")}__name(populateDemoData,"populateDemoData");function resetAllDataToZero(){if(!db)return;console.log("\u{1F9F9} Wiping all data to clean zero state in SQLite...");run("DELETE FROM transactions");run("DELETE FROM deleted_transactions");run("DELETE FROM categories");run("DELETE FROM prorated_rules");run("DELETE FROM savings_goals");run("DELETE FROM savings_history");run("DELETE FROM debts");run("DELETE FROM debt_payments");run("DELETE FROM recurring_items");run("DELETE FROM read_alerts");run("DELETE FROM user_settings");const initial=getInitialSeedData();for(const cat of initial.categories){run(`INSERT OR REPLACE INTO categories (id, name, icon, color, monthlyBudget, isCustom) VALUES (?, ?, ?, ?, ?, ?)`,[cat.id,cat.name,cat.icon,cat.color,cat.monthlyBudget||0,cat.isCustom?1:0])}run(`INSERT OR REPLACE INTO user_settings (id, currency, currencyCode, pushNotificationsEnabled, dailyBudgetAlertThreshold, monthlyBudgetWarningThreshold, enableRolloverByDefault, selectedMonth, userName)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,["default",initial.settings.currency,initial.settings.currencyCode,initial.settings.pushNotificationsEnabled?1:0,initial.settings.dailyBudgetAlertThreshold,initial.settings.monthlyBudgetWarningThreshold,initial.settings.enableRolloverByDefault?1:0,initial.settings.selectedMonth,initial.settings.userName]);persistDb();console.log("\u2705 Database reset to clean zero records.")}__name(resetAllDataToZero,"resetAllDataToZero");function seedIfEmpty(){if(!db)return;const rows=query("SELECT COUNT(*) as count FROM categories");if(rows[0]&&rows[0].count>0)return;console.log("\u{1F331} Database is empty. Seeding initial demo data...");const demo=getDemoSeedData();insertDemoData(demo);persistDb();console.log("\u2705 SQLite seeded with demo sample records.")}__name(seedIfEmpty,"seedIfEmpty");const transactionRepo={getAll(){const rows=query("SELECT * FROM transactions ORDER BY date DESC, created_at DESC");return rows.map(r=>({...r,amount:Number(r.amount),tags:JSON.parse(r.tags||"[]"),isRecurring:Boolean(r.isRecurring)}))},getById(id){const rows=query("SELECT * FROM transactions WHERE id = ?",[id]);if(rows.length===0)return null;const r=rows[0];return{...r,amount:Number(r.amount),tags:JSON.parse(r.tags||"[]"),isRecurring:Boolean(r.isRecurring)}},create(tx){run(`INSERT INTO transactions (id, title, amount, type, category, date, tags, notes, paymentMethod, isRecurring, recurringFrequency, receiptUrl)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,[tx.id,tx.title,tx.amount,tx.type,tx.category,tx.date,JSON.stringify(tx.tags||[]),tx.notes||null,tx.paymentMethod,tx.isRecurring?1:0,tx.recurringFrequency||null,tx.receiptUrl||null]);return tx},update(id,updates){const existing=this.getById(id);if(!existing)return null;const merged={...existing,...updates};run(`UPDATE transactions SET 
        title = ?, amount = ?, type = ?, category = ?, date = ?, 
        tags = ?, notes = ?, paymentMethod = ?, isRecurring = ?, 
        recurringFrequency = ?, receiptUrl = ?
       WHERE id = ?`,[merged.title,merged.amount,merged.type,merged.category,merged.date,JSON.stringify(merged.tags||[]),merged.notes||null,merged.paymentMethod,merged.isRecurring?1:0,merged.recurringFrequency||null,merged.receiptUrl||null,id]);return merged},delete(id){const existing=this.getById(id);if(existing){run(`INSERT INTO deleted_transactions (id, title, amount, type, category, date, tags, notes, paymentMethod, isRecurring, recurringFrequency, receiptUrl)
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,["default",merged.currency,merged.currencyCode,merged.pushNotificationsEnabled?1:0,merged.dailyBudgetAlertThreshold,merged.monthlyBudgetWarningThreshold,merged.enableRolloverByDefault?1:0,merged.selectedMonth,merged.userName]);return merged}};const readAlertsRepo={getAllReadIds(){const rows=query("SELECT alertId FROM read_alerts");return rows.map(r=>r.alertId)},markRead(alertId){run("INSERT OR IGNORE INTO read_alerts (alertId) VALUES (?)",[alertId])},markAllRead(alertIds){for(const id of alertIds){this.markRead(id)}},clearAll(){run("DELETE FROM read_alerts")}};function getDatabaseStats(){const txCount=query("SELECT COUNT(*) as count FROM transactions")[0]?.count||0;const catCount=query("SELECT COUNT(*) as count FROM categories")[0]?.count||0;const rulesCount=query("SELECT COUNT(*) as count FROM prorated_rules")[0]?.count||0;const savingsCount=query("SELECT COUNT(*) as count FROM savings_goals")[0]?.count||0;const debtCount=query("SELECT COUNT(*) as count FROM debts")[0]?.count||0;const recCount=query("SELECT COUNT(*) as count FROM recurring_items")[0]?.count||0;let fileSizeKb=0;if(fs.existsSync(DB_FILE)){try{const stat=fs.statSync(DB_FILE);fileSizeKb=Math.round(stat.size/1024)}catch{}}return{engine:"SQLite (sql.js WASM + File Persistence)",databaseFile:"data/budget.sqlite",fileSizeKb,tables:{transactions:txCount,categories:catCount,prorated_rules:rulesCount,savings_goals:savingsCount,debts:debtCount,recurring_items:recCount},status:"online",lastSync:new Date().toISOString()}}__name(getDatabaseStats,"getDatabaseStats");export{categoryRepo,debtRepo,deletedTransactionRepo,getDatabaseStats,initDatabase,populateDemoData,proratedRuleRepo,readAlertsRepo,recurringRepo,resetAllDataToZero,savingsRepo,settingsRepo,transactionRepo};
